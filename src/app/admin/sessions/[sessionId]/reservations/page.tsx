...
import {
  bookRosterStudentByStaffAction,
  markAllSessionStudentsAttendedAction,
  saveSessionJournalAction,
...
function autoResolveAbnormalStatus(
...
function compactSupplementSummary(
  detailCount: number,
  homeworkText: string,
  noteText: string,
) {
...
function isActiveEnrollmentStatus(status?: string) {
  const text = String(status ?? "")
    ...
    .toLowerCase();
  if (!text) return true;
  return ["active", "booked", "enrolled", "上課中", "已加入"].some((item) =>
    text.includes(item),
  );
}
...
function findOfferingForSession(
  course: Course,
...
function resolveEffectiveSessionTeachers(
  course: Course,
  session: CourseSession,
  offering: any,
  series: any,
  ...
  const primaryInstructorName =
    ...
    "";
  const assistantInstructorIds = session.assistantInstructorIds?.length
    ? session.assistantInstructorIds
    : offering?.assistantInstructorIds?.length
      ? offering.assistantInstructorIds
      : course.assistantInstructorIds?.length
        ? course.assistantInstructorIds
        : [];
  const assistantInstructorNamesFromIds = assistantInstructorIds
    .map((id: string) => getInstructorNameById(instructors, id))
    .filter(Boolean);
  const assistantInstructorNames = assistantInstructorNamesFromIds.length
    ? assistantInstructorNamesFromIds
    : session.assistantInstructorNames?.length
      ? session.assistantInstructorNames
      : offering?.assistantInstructorNames?.length
        ? offering.assistantInstructorNames
        : course.assistantInstructorNames?.length
...
function collectCourseSpecialtyKeywords(
  course: Course,
  offering: any,
  series: any,
  categories: any[],
  ...
  const category = categories.find(
    (item) => item?.id === sessionCategoryId(course, offering, series),
  );
  const rawText = [
    category?.name,
    category?.code,
  ...
  [
    ...
    if (normalizedText.includes(normalizeKeyword(keyword)))
  ...
  [
    ...
    .filter(
      (value) =>
        value.length >= 2 && !["all", "default", "other"].includes(value),
    )
...
function instructorMatchesCourse(
  instructor: any,
  course: Course,
  offering: any,
  series: any,
  ...
  if (
    directCourseMatches.includes(String(series?.id ?? "")) ||
    directCourseMatches.includes(String(offering?.id ?? "")) ||
    directCourseMatches.includes(String(course.seriesId ?? "")) ||
    directCourseMatches.includes(String(course.offeringId ?? ""))
  ) {
    return true;
  }
  ...
  return courseKeywords.some((courseKeyword) =>
    specialtyKeywords.some(
      (specialty) =>
        specialty.includes(courseKeyword) || courseKeyword.includes(specialty),
    ),
  );
}
...
function getEligibleInstructors(
  ...
  const courseKeywords = collectCourseSpecialtyKeywords(
    course,
    offering,
    series,
    categories,
...
export default async function AdminReservationsPage({
  ...
  const effectiveTeachers = resolveEffectiveSessionTeachers(
    course,
    session,
    offering,
    series,
  ...
  const eligibleInstructors = getEligibleInstructors(
    course,
    offering,
    series,
    instructors,
  ...
  const assistantNames =
    effectiveTeachers.assistantInstructorNames.filter(Boolean).join("、") ||
    "未設定";
    ...
    <AdminShell currentSection="attendance.dashboard">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <Link
              ...
              className="inline-flex text-xs font-bold text-[#8a7c72] hover:text-[#1f1712]"
            >
              ← 返回課堂日誌總覽
            </Link>
            <p className="mt-3 text-xs font-black text-[#B46F4A]">
              單堂課工作台
            </p>
            <h1 className="mt-1 text-3xl font-black tracking-tight text-[#1f1712]">
              {course.title}
            </h1>
          ...
          <div className="flex flex-wrap gap-2">
            <Link
              href="/admin/course-sessions"
              className="rounded-2xl border border-[#dbcabd] bg-white px-4 py-2.5 text-sm font-bold text-[#5A3726] hover:bg-[#fff6ed]"
            >
              返回總覽
            </Link>
          </div>
      ...
      <form action={saveSessionJournalAction} className="mb-4">
        ...
        <input
          type="hidden"
          name="redirectTo"
          value={`/admin/sessions/${encodedSessionId}/reservations#session-workspace`}
        />

        <section
          id="session-workspace"
          className="scroll-mt-24 rounded-[26px] border border-[#ead8ca] bg-white px-5 py-4 shadow-sm"
        >
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto_minmax(230px,0.46fr)] xl:items-center">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm leading-6 text-[#66584f]">
                ...
                <span>
                  地點：
                  <strong className="text-[#1f1712]">
                    {effectiveLocation || "未設定"}
                  </strong>
                </span>
                <span>
                  講師：
                  <strong className="text-[#1f1712]">
                    {effectiveTeachers.primaryInstructorName || "未設定"}
                  </strong>
                </span>
                <span>
                  助教：
                  <strong className="text-[#1f1712]">{assistantNames}</strong>
                </span>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-black text-[#5A3726] sm:grid-cols-5 xl:hidden">
                <span className="rounded-2xl border border-[#ead8ca] bg-[#fffdf9] px-3 py-2">
                  未確認 {unchecked}
                </span>
                ...
                <span className="rounded-2xl border border-violet-200 bg-violet-50 px-3 py-2 text-violet-700">
                  請假 {leave}
                </span>
              </div>
              <a
                href="#attendance-list"
                className="mt-4 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[#E85F00] text-sm font-black text-white shadow-sm hover:brightness-105 sm:w-auto xl:hidden"
              >
                前往點名
              </a>
            </div>
            ...
            <SessionInfoModalCard
              title="編輯課堂資料"
              description="調整日期、時間、單元、地點、講師與助教。"
            >
              <div className="grid gap-4 lg:grid-cols-2">
                <label className="grid gap-2 text-sm font-bold text-[#5A3726]">
                  ...
                  <input
                    ...
                    className="h-12 rounded-2xl border border-[#dbcabd] bg-white px-4 text-sm font-normal outline-none transition focus:border-[#E85F00] focus:ring-4 focus:ring-orange-100"
                  />
                </label>
                <label className="grid gap-2 text-sm font-bold text-[#5A3726]">
                  單元名稱
                ...
                <label className="grid gap-2 text-sm font-bold text-[#5A3726]">
                  ...
                  <select
                    name="instructorId"
                    defaultValue={effectiveTeachers.primaryInstructorId}
                    className="h-12 rounded-2xl border border-[#dbcabd] bg-white px-4 text-sm font-black text-[#1f1712] outline-none transition focus:border-[#E85F00] focus:ring-4 focus:ring-orange-100"
                  >
                ...
                <div className="grid gap-2 text-sm font-bold text-[#5A3726]">
                  ...
                  <div className="flex min-h-12 flex-wrap items-center gap-2 rounded-2xl border border-[#dbcabd] bg-white px-3 py-2">
                    {eligibleInstructors.map((instructor) => (
                      <label
                        key={instructor.id}
                        className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-[#ead8ca] bg-[#fffaf5] px-3 py-2 text-xs font-bold text-[#5A3726] transition hover:border-[#E85F00] has-[:checked]:border-[#E85F00] has-[:checked]:bg-[#fff1e7] has-[:checked]:text-[#E85F00]"
                      >
                        <input
                          type="checkbox"
                          name="assistantInstructorIds"
                          value={instructor.id}
                          defaultChecked={effectiveTeachers.assistantInstructorIds.includes(
                            instructor.id,
                          )}
                    ...
                    {eligibleInstructors.length === 0 ? (
                      <span className="text-sm font-normal text-[#8a7c72]">
                        目前沒有符合本課程專業項目的講師可選，請先到{" "}
                        <Link
                          href="/admin/students?mode=instructors"
                          className="font-black text-[#E85F00] underline"
                        >
                          名冊資料／講師名冊
                        </Link>{" "}
                        補上講師專業項目。
              ...
              <div className="mt-5 grid gap-2 text-sm font-bold text-[#5A3726] lg:col-span-2">
                課堂狀態
                <div className="grid gap-2 rounded-2xl border border-[#ead8ca] bg-[#fffdf9] p-3 sm:grid-cols-5">
                  {[
                    {
                      value: "scheduled",
                      label: "正常上課",
                    ...
                    { value: "suspended", label: "停課", hint: "暫停本堂課" },
                    { value: "makeup", label: "補課", hint: "補課紀錄" },
                    { value: "rescheduled", label: "調課", hint: "已調整日期" },
                    {
                      value: "cancelled",
                      label: "已取消",
                      hint: "學生端不可預約",
                    ...
                    <label
                      key={option.value}
                      className="cursor-pointer rounded-2xl border border-[#ead8ca] bg-white p-3 text-xs font-black text-[#5A3726] transition has-[:checked]:border-[#E85F00] has-[:checked]:bg-[#fff1e7] has-[:checked]:text-[#E85F00]"
                    >
                      <input
                        type="radio"
                        name="status"
                        value={option.value}
                        defaultChecked={sessionStatus === option.value}
                        className="sr-only"
                      />
                      <span className="block">{option.label}</span>
                      <span className="mt-1 block text-[11px] font-semibold text-[#8a7c72]">
                        {option.hint}
                      </span>
                    </label>
                  ))}
                </div>
                <p className="text-xs font-normal leading-5 text-[#8a7c72]">
                  停課或已取消後，學生端不可預約；後台仍會保留課堂紀錄。
                </p>
              </div>
            ...
            <div className="grid gap-2 rounded-[20px] border border-[#f1e2d6] bg-[#fffaf5] px-4 py-3 text-sm text-[#66584f]">
              <div className="flex items-center justify-between gap-3">
                ...
                <strong
                  className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${sessionStatusBadgeClass(sessionStatus)}`}
                >
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <strong
                  className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${attendanceWorkflowBadgeClass(attendanceStatus)}`}
                >
                  {attendanceWorkflowLabel(attendanceStatus)}
                </strong>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs font-bold">TTQS</span>
                <strong
                  className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${abnormalStatusTone(abnormalResolvedStatus)}`}
                >
          ...
          <div className="mt-4 border-t border-[#f5e8dc] pt-4">
            <div className="grid gap-3 xl:grid-cols-[1fr_1fr_0.75fr]">
              ...
              <div className="grid gap-2 text-xs font-black text-[#B46F4A]">
                ...
                <input
                  type="hidden"
                  name="abnormalResolvedStatus"
                  value={abnormalResolvedStatus}
                />
                <div
                  className={`flex min-h-10 items-center rounded-[18px] border px-3 py-2 text-xs font-black ${abnormalStatusTone(abnormalResolvedStatus)}`}
                >
                  {abnormalLabel(abnormalResolvedStatus)}
      ...
      <div className="flex flex-col">
        <details
          id="lesson-journal"
          className="scroll-mt-24 order-2 mb-4 rounded-[24px] border border-[#ead8ca] bg-white shadow-sm xl:order-1"
        >
        ...
        <section
          id="attendance-list"
          className="scroll-mt-24 order-1 mb-6 rounded-[26px] border border-[#ead8ca] bg-white p-4 shadow-sm sm:p-5 xl:order-2"
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
          ...
          {attendanceRows.length === 0 ? (
            ...
            <div className="mt-5 rounded-[22px] border border-[#ead8ca] bg-[#fffaf5] md:overflow-x-auto md:bg-white">
                  ...
                  <div key={reservation.id}>
                    <div className="m-1.5 rounded-[18px] border border-[#ead8ca] bg-white px-3 py-2.5 shadow-sm md:hidden">
                      <div className="flex min-w-0 items-center gap-2">
                        <p className="min-w-0 flex-1 truncate text-base font-black text-[#1f1712]">
                          {reservation.studentName}
                        </p>
                        <span
                          className={`inline-flex shrink-0 rounded-full border px-2 py-0.5 text-[11px] font-black ${attendanceStatusCompactClass(displayStatus)}`}
                        >
                          {attendanceStatusText(displayStatus)}
                        </span>
                        {reservation.isRosterOnly ? (
                          <form
                            action={bookRosterStudentByStaffAction}
                            className="shrink-0"
                          >
                            <input
                              type="hidden"
                            ...
                            <input
                              type="hidden"
                              name="redirectTo"
                              value={`${currentUrl}#attendance-list`}
                            />
                            <button
                              type="submit"
                      ...
                      <details className="mt-2 border-t border-[#f1e2d6] pt-1.5">
                        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 py-1.5 text-xs font-black text-[#8B4A2F] marker:hidden">
                          <span>
                            {supplementSummary === "無補充"
                              ? "＋ 補充資料"
                              : "補充資料"}
                          </span>
                          <span className="min-w-0 truncate text-right font-bold text-[#8a7c72]">
                            {supplementSummary}
                          </span>
                        </summary>
                        <div className="grid gap-2 pt-2">
                          <div className="rounded-[14px] border border-[#f1e2d6] bg-[#fffaf5] px-3 py-2 text-xs font-bold leading-6 text-[#B46F4A]">
                            <p className="font-black text-[#B46F4A]">
                              出席補充
                            </p>
                    ...
                    <div className="hidden md:grid md:min-w-[1180px] md:grid-cols-[0.68fr_minmax(410px,2.2fr)_1fr_0.95fr_0.95fr] md:items-stretch md:border-b md:border-[#ead8ca] md:bg-white md:px-4 md:py-3">
                      <div>
                        <p className="text-base font-black text-[#1f1712]">
                          {reservation.studentName}
                        </p>
                        {reservation.isRosterOnly ? (
                          <div className="mt-2 grid gap-2">
                            <p className="text-xs font-bold text-[#8a7c72]">
                              由名冊帶入，尚未正式預約
                            </p>
              ...
              {displayedReservations.length === 0 ? (
                <div className="px-5 py-8 text-sm leading-6 text-[#8a7c72]">
                  目前沒有符合篩選條件的學員。
                </div>
              ) : null}
            </div>
      ...
      <details className="fixed bottom-4 right-4 z-40 xl:hidden">
        ...
        <div className="absolute bottom-16 right-0 w-64 overflow-hidden rounded-[22px] border border-[#ead8ca] bg-white p-2 shadow-2xl shadow-orange-950/15">
          <p className="px-3 py-2 text-xs font-black text-[#B46F4A]">
            快速操作
          </p>
          <a
            href="#session-workspace"
            className="block rounded-2xl px-3 py-3 text-sm font-black text-[#1f1712] hover:bg-[#fff6ed]"
          >
            回到課堂摘要
          </a>
          <a
          ...
          <form action={markAllSessionStudentsAttendedAction}>
            ...
            <button
              type="submit"
              className="mt-1 w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-left text-sm font-black text-emerald-700 hover:bg-emerald-100"
            >
              一鍵全到
            </button>
          </form>
          <form action={saveSessionJournalAction}>
            ...
            <button
              type="submit"
              className="mt-1 w-full rounded-2xl border border-[#ead8ca] bg-[#fffaf5] px-3 py-3 text-left text-sm font-black text-[#5A3726] hover:bg-[#fff6ed]"
            >
              完成點名
            </button>
          </form>
