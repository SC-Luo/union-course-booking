...
export default async function WeeklyBookingsPage() {
  const { today, weeklySessions } = await getDashboardData();
  const totalBooked = weeklySessions.reduce((sum, item) => sum + item.bookedCount, 0);
  const totalCapacity = weeklySessions.reduce((sum, item) => sum + item.capacity, 0);
  const fullCount = weeklySessions.filter((item) => item.capacity > 0 && item.bookedCount >= item.capacity).length;

  return (
    <AdminShell currentSection="dashboard.weekly-booking">
      <PageHero
        label="工作台"
        title="本週報名狀況"
        description="獨立檢查未來 7 天內會上課的場次，確認名額、報名人數、名冊人數與是否即將鎖定。"
        action={
          <>
            <Link href="/admin/booking-locks" className="rounded-2xl border border-[#B46F4A]/25 bg-white/70 px-4 py-3 text-sm font-semibold text-[#5A3726] hover:bg-[#fff6ed]">查看即將鎖定</Link>
            <Link href="/admin/courses?section=offering" className="rounded-2xl bg-gradient-to-br from-[#E85F00] via-[#E7892B] to-[#B46F4A] px-4 py-3 text-sm font-semibold text-white shadow-sm hover:brightness-95">管理年度期別</Link>
          </>
        }
      />

      <section className="mb-8 grid gap-4 md:grid-cols-3">
        <article className="rounded-[28px] border border-[#ead8ca] bg-[#fffdf9] p-5 shadow-[0_12px_36px_rgba(90,55,38,0.06)]"><p className="text-sm font-semibold text-[#B46F4A]">本週場次</p><p className="mt-3 text-4xl font-black text-[#1f1712]">{weeklySessions.length}</p><p className="mt-2 text-sm text-[#8a7c72]">未來 7 天內排定</p></article>
        <article className="rounded-[28px] border border-[#ead8ca] bg-[#fffdf9] p-5 shadow-[0_12px_36px_rgba(90,55,38,0.06)]"><p className="text-sm font-semibold text-[#B46F4A]">報名名額</p><p className="mt-3 text-4xl font-black text-[#1f1712]">{totalBooked}/{totalCapacity}</p><p className="mt-2 text-sm text-[#8a7c72]">已報名 / 總名額</p></article>
        <article className="rounded-[28px] border border-amber-200 bg-[#fffaf0] p-5 shadow-[0_12px_36px_rgba(90,55,38,0.06)]"><p className="text-sm font-semibold text-[#8B5035]">已額滿</p><p className="mt-3 text-4xl font-black text-[#1f1712]">{fullCount}</p><p className="mt-2 text-sm text-[#8a7c72]">需要行政留意</p></article>
      </section>
      ...
      <section className="rounded-[30px] border border-[#ead8ca] bg-[#fffdf9] p-6 shadow-[0_16px_45px_rgba(90,55,38,0.07)] sm:p-7">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div><p className="text-sm font-semibold text-[#B46F4A]">報名列表</p><h2 className="mt-1 text-2xl font-black text-[#1f1712]">未來 7 天場次</h2></div>
          <span className="rounded-full bg-[#f5ece4] px-3 py-1 text-sm font-medium text-[#66584f]">今天：{formatDate(today)}</span>
        </div>
        <div className="mt-5 grid gap-3">
          {weeklySessions.length === 0 ? <p className="rounded-2xl bg-[#fff9f3] p-5 text-sm text-[#8a7c72]">未來 7 天內沒有排定上課場次。</p> : null}
          {weeklySessions.map(({ course, series, session, color, categoryName, yearTermLabel, bookedCount, capacity, rosterCount }) => {
            const isFull = capacity > 0 && bookedCount >= capacity;
            return (
              <Link key={`${course.id}-${session.id}`} href={`/admin/sessions/${session.id}/reservations`} className="grid gap-4 rounded-2xl border border-[#eaded3] bg-white p-4 transition hover:-translate-y-0.5 hover:shadow-md lg:grid-cols-[minmax(0,1fr)_360px_140px] lg:items-center" style={{ borderLeftColor: color, borderLeftWidth: 5 }}>
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#f5ece4] px-2 py-1 text-xs font-semibold text-[#4e4038]">{categoryName}</span><span className="rounded-full bg-[#fff6ed] px-2 py-1 text-xs font-medium text-[#B46F4A]">{yearTermLabel}</span><span className="rounded-full bg-[#fffaf0] px-2 py-1 text-xs font-semibold text-[#8B5035]">{formatLockLabel(today, session.date)}</span></div>
                  <h3 className="mt-3 text-lg font-bold text-[#1f1712]">{series.title}</h3>
                  <p className="mt-1 text-sm text-[#66584f]">{formatDate(session.date)} {session.startTime}–{session.endTime}｜{session.location || course.defaultLocation || "未設定地點"}</p>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm"><div className="rounded-xl bg-[#fff9f3] p-3"><p className="text-xs text-[#8a7c72]">報名</p><p className="mt-1 text-lg font-semibold text-[#1f1712]">{bookedCount}/{capacity}</p></div><div className="rounded-xl bg-[#fff9f3] p-3"><p className="text-xs text-[#8a7c72]">名冊</p><p className="mt-1 text-lg font-semibold text-[#1f1712]">{rosterCount}</p></div><div className={isFull ? "rounded-xl bg-[#fff3e8] p-3" : "rounded-xl bg-[#fff9f3] p-3"}><p className="text-xs text-[#8a7c72]">狀態</p><p className="mt-1 text-lg font-semibold text-[#1f1712]">{isFull ? "已滿" : `${Math.max(capacity - bookedCount, 0)} 空位`}</p></div></div>
