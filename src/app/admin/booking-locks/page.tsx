...
export default async function BookingLocksPage() {
  ...
  const todayLocking = lockingSessions.filter((item) => item.daysUntil <= 0).length;

  return (
    <AdminShell currentSection="dashboard.booking-lock">
      <PageHero
        label="工作台"
        title="即將鎖定預約"
        description="依目前規則，上課前一週內學員不可自行更改預約。這一頁用來讓行政人員提前確認名額、名冊與異常狀況。"
        action={<Link href="/admin/weekly-bookings" className="rounded-2xl bg-gradient-to-br from-[#E85F00] via-[#E7892B] to-[#B46F4A] px-4 py-3 text-sm font-semibold text-white shadow-sm hover:brightness-95">查看本週報名</Link>}
      />

      <section className="mb-8 grid gap-4 md:grid-cols-3">
        <article className="rounded-[28px] border border-[#ead8ca] bg-[#fffdf9] p-5 shadow-[0_12px_36px_rgba(90,55,38,0.06)]"><p className="text-sm font-semibold text-[#B46F4A]">即將鎖定</p><p className="mt-3 text-4xl font-black text-[#1f1712]">{lockingSessions.length}</p><p className="mt-2 text-sm text-[#8a7c72]">7 天內場次</p></article>
        <article className="rounded-[28px] border border-amber-200 bg-[#fffaf0] p-5 shadow-[0_12px_36px_rgba(90,55,38,0.06)]"><p className="text-sm font-semibold text-[#8B5035]">今天鎖定</p><p className="mt-3 text-4xl font-black text-[#1f1712]">{todayLocking}</p><p className="mt-2 text-sm text-[#8a7c72]">今天需要特別確認</p></article>
        <article className="rounded-[28px] border border-[#ead8ca] bg-[#fffdf9] p-5 shadow-[0_12px_36px_rgba(90,55,38,0.06)]"><p className="text-sm font-semibold text-[#B46F4A]">檢查日期</p><p className="mt-3 text-4xl font-black text-[#1f1712]">7</p><p className="mt-2 text-sm text-[#8a7c72]">天內不可更改提醒</p></article>
      </section>

      <section className="rounded-[30px] border border-[#ead8ca] bg-[#fffdf9] p-6 shadow-[0_16px_45px_rgba(90,55,38,0.07)] sm:p-7">
        <div><p className="text-sm font-semibold text-[#B46F4A]">鎖定列表</p><h2 className="mt-1 text-2xl font-black text-[#1f1712]">需要提前確認的場次</h2><p className="mt-1 text-sm text-[#8a7c72]">建議確認：報名人數、候補需求、名冊資料、講師是否已確認。</p></div>
        <div className="mt-5 grid gap-3">
          {lockingSessions.length === 0 ? <p className="rounded-2xl bg-[#fff9f3] p-5 text-sm text-[#8a7c72]">目前沒有即將鎖定的場次。</p> : null}
          {lockingSessions.map(({ course, series, session, color, categoryName, yearTermLabel, bookedCount, capacity, rosterCount, daysUntil }) => (
            <Link key={`${course.id}-${session.id}`} href={`/admin/sessions/${session.id}/reservations`} className="grid gap-4 rounded-2xl border border-[#eaded3] bg-white p-4 transition hover:-translate-y-0.5 hover:shadow-md lg:grid-cols-[minmax(0,1fr)_330px_140px] lg:items-center" style={{ borderLeftColor: color, borderLeftWidth: 5 }}>
              <div>
                <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#fff6ed] px-2 py-1 text-xs font-semibold text-[#B46F4A]">{formatLockLabel(today, session.date)}</span><span className="rounded-full bg-[#f5ece4] px-2 py-1 text-xs font-semibold text-[#4e4038]">{categoryName}</span><span className="rounded-full bg-[#fffaf0] px-2 py-1 text-xs font-semibold text-[#8B5035]">{yearTermLabel}</span></div>
              ...
              <div className="grid grid-cols-3 gap-2 text-sm"><div className="rounded-xl bg-[#fff9f3] p-3"><p className="text-xs text-[#8a7c72]">報名</p><p className="mt-1 text-lg font-semibold text-[#1f1712]">{bookedCount}/{capacity}</p></div><div className="rounded-xl bg-[#fff9f3] p-3"><p className="text-xs text-[#8a7c72]">名冊</p><p className="mt-1 text-lg font-semibold text-[#1f1712]">{rosterCount}</p></div><div className="rounded-xl bg-[#fff9f3] p-3"><p className="text-xs text-[#8a7c72]">剩餘</p><p className="mt-1 text-lg font-semibold text-[#1f1712]">{daysUntil === 0 ? "今天" : `${daysUntil} 天`}</p></div></div>
              <span className="inline-flex items-center justify-center rounded-xl bg-[#5A3726] px-4 py-3 text-sm font-semibold text-white hover:bg-[#8B5035]">確認名單</span>
            </Link>
          ))}
        </div>
      </section>
