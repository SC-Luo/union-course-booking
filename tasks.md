...
- [ ] **(優先任務) 持續進行功能測試（包含前台預約、取消及後台名冊操作）。**
...
- [x] 2026-07-23 進行 Firestore 讀取量優化，實作在線記憶體快取（In-Memory Cache）與管理端自動失效清空，並重構 getBookingData 支援 skip 窄化讀取參數，成功阻斷公開前台首頁及後台（統計、年度課程、時段）的高負載 document reads。
