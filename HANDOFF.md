...
- 2026-08-05 修復後台課程調整 production server error：`/admin/course-offerings` 與 `/admin/course-masters` 曾因 Firestore payload 含 `undefined` 欄位（如 `code`、`defaultCourseMode`）而寫入失敗。已在 `src/lib/booking-repository.ts` 統一清理 Firestore `.set()` payload，並把原因寫入踩坑紀錄。
