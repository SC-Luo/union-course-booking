...
### 同一週期同一課程群組同一學員只能預約一個時段

為提供彈性排課並預防資源獨占，針對週期預約課（如美甲、美容輔導班），放寬原「一門課僅能預約一次」的硬性限制，決定引入「週期限制預約」政策：
- **預約週期起迄**：統一採用 **週日到週六** 為一個週期區間（例如週日 8/2 到週六 8/8 歸為同一週期 `2026-08-02~2026-08-08`）。
- **課程群組控制**：以 `bookingQuotaGroupId` 欄位作為群組 ID。
- **有效預約限制**：在同一個預約週期與同一個課程群組下，學員僅能持有一筆狀態為 `booked` 的有效預約。取消（`cancelled`）的預約不計入額度阻擋。學員在下一個預約週期（下個週日到週六）可再度進行預約。
...
正式上線後，Firestore 是唯一的真實資料源。
- 本地 `data/booking-data.json` 已從 Git 中移出追蹤，防止開發測試所使用的學員個資意外推上 GitHub，並以去識別化的 `data/booking-data.example.json` 作為範本。
- 日常開發本地預設以 `json` 離線開發模式運行，避免本地 `npm run dev` 誤改正式資料庫。
- 敏感寫入工具已全面加載三層確認鎖、強制線上快照備份與節流，防止任何日常部署流程誤觸。
