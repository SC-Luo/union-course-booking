...
目前確認 Firestore 資料庫內所有學員資料均為模擬/測試資料，不具備真實生產個資。因此，可直接實作嚴格的新版學員完整度檢核規則（將生日、完整身分證號列為必填檢核項目），無須擔心現有真實學員因此被降級而造成行政困擾。
- **後續清理**：在系統正式上線生產環境 (Production) 之前，工作人員必須徹底清空資料庫中的測試學員與測試預約資料，並依循最新定義的學員完整建檔條件與核對導入流程重新執行正式學員資料導入。
